...
export function initStudyMotion() {
  ...
  const mount = () => {
    ...
    media.add("(prefers-reduced-motion: no-preference)", () => {
      ...
      if (footer) {
        const timeline = gsap.timeline({ scrollTrigger: { trigger: footer, start: "top 92%", once: true } });
        timeline.from(footer.querySelector("[data-study-footer-line]"), { scaleX: 0, transformOrigin: "0 50%", duration: 1.5, ease: "power3.inOut" })
          .from(footer.querySelectorAll("[data-study-footer-item]"), { y: 64, opacity: 0, duration: 1.25, stagger: .13, ease: "power3.out", clearProps: "transform,opacity" }, .2);
      }
    });
  };
  let bound = false;
